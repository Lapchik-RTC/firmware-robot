...
#define ENC_MASK 0b00001100 // удобное для работы с таблицей обозначение положения, полученного
// с датчика холла
...
float tick = 0; // угол поворота вала в тиках в данный момент, который возможно не будет использоваться
float tick_to_tick = 0; // коэффициент пересчёта в тики
float w_tick = 0;  // текущая скорость в тики/c
float phi = 0; // угол поворота вала в радианах в данный момент, который возможно не будет использоваться
...
void encoder_tick()
{
    ...
    phi = counter_inc * tick_to_rad; // или тут должно быть "+="?
    tick = counter_inc * (KOLVO_ENC_TICK * GEAR_RATIO);  // или тут должно быть "+="?
