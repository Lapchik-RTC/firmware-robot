...
int main() {
    ...
    cout << L"Программа запущена. Нажмите ESC для выхода...\n";
    ...
    while (true) {
        ...
        buffer[1] = 1;//|= state.A << 7;          //кнопка А
        buffer[1] = 1;//|= state.B << 6;          //кнопка В
        buffer[1] = 1;//|= state.X << 5;          //кнопка Х
        buffer[1] = 1;//|= state.Y << 4;          //кнопка Y
        buffer[1] = 1;//|= state.DPad_Up << 3;    // DPad вверх
        buffer[1] = 1;//|= state.DPad_Down << 2;  // DPad вниз
        buffer[1] = 1;//|= state.DPad_Left << 1;  // D Pad влево
        buffer[1] = 1;//|= state.DPad_Right;      // D Pad вправо

        // Стики
        buffer[2] = 1;//(state.LeftThumbX >> 8) & 0xFF;     //  левый стик Х
        buffer[3] = 1;//state.LeftThumbX & 0xFF;            //  левый стик Х (младшие)
        buffer[4] = 1;//(state.LeftThumbY >> 8) & 0xFF;     //  левый стик Y
        buffer[5] = 1;// state.LeftThumbY & 0xFF;            //  левый стик Y (младшие)

        buffer[6] = 1;// (state.RightThumbX >> 8) & 0xFF;    //  правый стик Х
        buffer[7] = 1;//state.RightThumbX & 0xFF;           //  правый стик Х (младшие)
        buffer[8] = 1;//(state.RightThumbY >> 8) & 0xFF;    //  правый стик Y
        buffer[9] = 1;//state.RightThumbY & 0xFF;           //  правый стик Y (младшие)

        // Байт 10: дополнительные кнопки
        buffer[10] =1;//|= state.LeftThumb << 7;
        buffer[10] =1;//|= state.RightThumb << 6;
        buffer[10] =1;//|= state.LB << 5;
        buffer[10] =1;//|= state.RB << 4;
        buffer[10] =1;//|= (state.LeftTrigger > 0) << 3;
        buffer[10] =1;//|= (state.RightTrigger > 0) << 2;
        buffer[10] =1;//|= state.Start << 1;
        buffer[10] =1;//|= state.Back;
